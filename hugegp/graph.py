...
def check_graph(points, neighbors, offsets, indices):
    ...
    assert jnp.all(max_neighbors < offsets[offsets_index]), "Neighbors must be coarse points only in strict mode"

def make_offsets(n_points, n_initial, factor=2.0, max_batch=None):
    offsets = [n_initial]
    while offsets[-1] < n_points:
        next = offsets[-1] * factor
        if max_batch is not None:
            next = min(next, offsets[-1] + max_batch)
        offsets.append(int(min(next, n_points)))
    return tuple(offsets)

def build_jax_graph(points, *, offsets, k, cuda=False):
    n_points = len(points)
