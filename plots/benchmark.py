...
N_VALUES = [1e6, 3e6, 1e7, 3e7, 1e8, 3e8, 1e9] #[1e6, 3e6]#, 1e7, 3e7, 1e8, 3e8, 1e9]
...
def _worker(n: int, k: int, cuda: bool, result_path: str):
    """Subprocess entry point: runs one benchmark and writes the result to a JSON file."""
    result = run_benchmark(n, k, cuda)
    with open(result_path, "w") as f:
        json.dump(result, f)


def run_isolated(n: int, k: int, cuda: bool) -> dict:
    """Run a single benchmark in a fresh subprocess so CUDA memory is fully released afterwards."""
    fd, result_path = tempfile.mkstemp(suffix=".json")
    os.close(fd)
    try:
        ctx = mp.get_context("spawn")
        p = ctx.Process(target=_worker, args=(n, k, cuda, result_path))
        p.start()
        p.join()
        try:
            ...
            return {"n": n, "k": k, "cuda": cuda, "status": "crash", "error": f"exitcode={p.exitcode}"}
    finally:
...
def main():
    ...
    parser.add_argument(
        "--output",
        type=str,
        default="output/simple_results_32.json",
        help="Output JSON file (default: output/simple_results.json)",
    )
    parser.add_argument(
        "--continue",
        dest="continue_run",
