...
ax.plot(tree_batches, label='Tree', c='C2')
ax.plot(graphgp_batches, label='GraphGP', c='C0')
ax.plot(random_batches, label='Random', c='k', zorder=-1)
# ax.plot(mega_graphgp_batches, label='GraphGP (10M)', c='gray', zorder=-1)
# ax.plot(jnp.arange(jnp.log2(n_points)), 2**jnp.arange(jnp.log2(n_points)), '--', label='Doubling', c='gray')
ax.set(box_aspect=1, yscale='log', xlabel='Batch', ylabel='Batch size', xlim=(0, len(tree_batches)), ylim=(1, 1e3), title='Parallelism by order')
ax.legend(loc='upper right', fontsize=8)

plt.savefig("../../Overleaf/graphgp-pai26/figures/level_sizes_by_order.pdf", bbox_inches='tight')
plt.show()
...
kl_matrix = jnp.zeros((len(k_scan), 3))

for order in ['graphgp', 'random', 'tree']:
    for j, k in enumerate(k_scan):
        ...
        kl_matrix = kl_matrix.at[j, ['graphgp', 'tree', 'random'].index(order)].set(kl_per_dof)
        print(f"Order={order}, k={k}, KL/dof={kl_per_dof:.3e}")

#%%
fig, ax = plt.subplots(figsize=(4,4), dpi=300)
for i, order in enumerate(['graphgp', 'tree', 'random']):
    ax.plot(jnp.log(k_scan), kl_matrix[:,i], marker='o', label={'graphgp':'GraphGP', 'tree':'Tree', 'random':'Random'}[order], c={'graphgp':'C2', 'tree':'C0', 'random':'k'}[order])
ax.set(xlabel='Neighbors $k$', ylabel='KL / dof', title='Accuracy by order', xlim=(0,3), xticks=jnp.log(k_scan), xticklabels=k_scan, yscale='log')
ax.legend()

plt.savefig("../../Overleaf/graphgp-pai26/figures/kl_by_order.pdf", bbox_inches='tight')
plt.show()
