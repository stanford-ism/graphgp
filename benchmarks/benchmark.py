...
- "serial_depth": ignored (kept for config compatibility)
"""
...
from graphgp.tree import build_tree, query_preceding_neighbors
from graphgp.graph import compute_depths, order_by_depth
...
def run_single_benchmark(test_params):
    ...
    covariance = gp.extras.matern_kernel(
        ...
        variance=test_params["covariance"].get("variance", 1.0),
        cutoff=test_params["covariance"].get("cutoff", 1.0),
    ...
    if test_params["graph"]["strict"]:
        if test_params["graph"]["fuse"]:
            ...
            for i in range(2):
                start = time.perf_counter()
                depths = compute_depths(neighbors, n0=test_params["n0"], cuda=test_params["cuda"])
    ...
    if test_params["function"] == "forward":
        ...
        func = jax.jit(lambda g, c, xi: jax.jvp(lambda x: gp.generate(g, c, x, cuda=test_params["cuda"]), (xi,), (jnp.ones_like(xi),))[1])
    elif test_params["function"] == "vjp":
        func = jax.jit(lambda g, c, xi: jax.vjp(lambda x: gp.generate(g, c, x, cuda=test_params["cuda"]), xi)[1](jnp.ones_like(xi))[0])
